...
import de.bytestore.lib.Callbacks.*;
import de.bytestore.lib.Networking.HttpHelper;
...
public class Client {
    ...
    public void inviteRoom(String roomID, String displayIO, String reasonIO, DataCallback onJoined) throws IOException {
        if (!isLoggedIn)
            return;

        JSONObject jsonObject = new JSONObject();
        jsonObject.put("displayname", displayIO);
        jsonObject.put("membership", "invite");
        jsonObject.put("reason", reasonIO);

        httpHelper.sendRequestAsync(host, HttpHelper.URLs.rooms + roomID + "/invite", null, "POST", data -> {
            if (onJoined != null) {
                onJoined.onData(data);
            }
        });
    ...
    public void nickname(StringBuilder displayIO) {
        if (!isLoggedIn)
            return;
